...
window.onload = function(){
	...
	btn = document.getElementById("deletePic").addEventListener("click", deletePic);
}
...
function deletePic() {
	...
	webRequest.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			document.getElementById("deleteInf").innerHTML = this.responseText;
		} 
	};
	webRequest.open("GET", "deleteUsersPhotos.php?photoid=" + photoId, true);
	webRequest.send();
}